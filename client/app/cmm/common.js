...
app.provider('routeResolver', [function() {
	...
	this.resolve = function(data) {
		var o = {};
		o.controller = data.name;
		o.controllerAs = data.as;
		o.templateUrl = WEB_ROOT + '/views' + data.tpl + '.tpl';
		o.resolve = {};
		return o;
	}
}]);
...
app.factory('logSvc', ['$log', function($log) {
	...
	o.getLogger = function(logNm) {
		var logger = {
			logNm: logNm,
			out: function() {
				$log.debug.apply($log, _.flatten(['%c ' + logNm + ' ', 'background: #f00; color: #fff', _.toArray(arguments)]));
			}
		};
		return logger;
	}
...
app.factory('utilSvc', [function() {
	...
	o.formatDate = function formatDate(d, f) {
		...
		if (_.isDate(d)) {
			return f.replace(/(yyyy|yy|mm|dd|hh24|hh|mi|ss|a\/p)/gi, function($1) {
				switch ($1) {
					case "yyyy":
